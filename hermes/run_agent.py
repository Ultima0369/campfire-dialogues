...
class AIAgent:
    ...
    def __init__(
        ...
        if self.api_mode == "anthropic_messages":
            ...
            # Inject custom httpx client that ignores system proxy settings.
            # When desktop environment (GNOME) configures a SOCKS proxy via GSettings,
            # httpx will try to use it. Clash Verge's mixed-port (25841) serves both
            # HTTP and SOCKS, but aiohttp-socks expects 'socks5://' scheme, not 'socks://'.
            # Setting trust_env=False forces direct connection, bypassing all proxy env
            # and desktop environment proxy configurations.
            import httpx
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        # If content was promoted from reasoning (StepFun-style responses),
        # suppress the separate reasoning callback to avoid double-display.
        _reasoning_promoted = getattr(assistant_message, '_reasoning_promoted', False)
        if _reasoning_promoted:
            # The answer is already in content; don't treat reasoning as thinking.
            reasoning_text = ""

        if reasoning_text and self.reasoning_callback and not _reasoning_promoted:
    ...
    def run_conversation(
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            try:
                ...
                # StepFun compatibility: Some models (e.g. step-3.5-flash-2603) return
                # the actual answer in the `reasoning` field with `content` empty.
                # Promote reasoning to content so the rest of the agent logic treats it as a
                # visible response. The _reasoning_promoted flag prevents double
                # display later in _build_assistant_message.
                if not assistant_message.content:
                    reasoning_val = getattr(assistant_message, 'reasoning', None) or getattr(assistant_message, 'reasoning_content', None)
                    if reasoning_val:
                        assistant_message.content = reasoning_val
                        assistant_message._reasoning_promoted = True
