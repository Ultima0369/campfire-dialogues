...
class TestRunConversation:
    ...
    def test_reasoning_only_local_resumed_no_compression_triggered(self, agent):
        ...
        # With StepFun-style reasoning-only responses, the answer is in reasoning_content.
        # The agent now accepts it directly without retries.
        with (
        ...
        assert result["api_calls"] == 1  # accepted without retries

    def test_reasoning_only_response_prefill_then_empty(self, agent):
        """Structured reasoning-only with content=None is now treated as a complete answer."""
        self._setup_agent(agent)
        empty_resp = _mock_response(
            content=None,
            finish_reason="stop",
        ...
        # Single response; reasoning_content is promoted to content and accepted.
        agent.client.chat.completions.create.side_effect = [empty_resp]
        with (
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
        ...
        assert result["api_calls"] == 1  # no retries needed

    def test_reasoning_only_prefill_succeeds_on_continuation(self, agent):
        """When first response has only reasoning_content, it's accepted as final answer."""
        self._setup_agent(agent)
        ...
        # Only one call needed; the reasoning_content is promoted to content.
        agent.client.chat.completions.create.side_effect = [reasoning_resp]
        with (
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ...
        assert result["completed"] is True
        assert result["final_response"] == "structured reasoning answer"
        assert result["api_calls"] == 1  # no continuation needed
